...
def test_breweries_meta_data(breweries_api_user):
    expected_res = {"total": "8237", "page": "1", "per_page": "50"}
...
def test_breweries_existence_by_country(breweries_api_user, country):
    breweries_response_by_country = breweries_api_user.get_data(
        "?by_country=" + country, None
    ).json()
    breweries_response_by_country_metadata = breweries_api_user.get_data(
        "meta?by_country=" + country, None
    ).json()
    assert breweries_response_by_country_metadata["total"] == str(
        len(breweries_response_by_country)
    )
...
def test_list_get_random_breweries(breweries_api_user):
    per_page = 3
    list_breweries_response = breweries_api_user.get_data(f"?per_page={per_page}", None)
    assert list_breweries_response.status_code == 200
    assert len(list_breweries_response.json()) == per_page
